...
| 10 | [I2C address change](#scenario-10--i2c-address-change-develop-mode) | low | [`05_AddressChange.ino`](../examples/05_AddressChange/05_AddressChange.ino) |
...
unique I2C address — use [scenario 10](#scenario-10--i2c-address-change-develop-mode)
to assign addresses during installation.

> v1 firmware note: `RbAmp::broadcastLatch()` returns `false` without
> touching the wire (SPEC §9 — General-Call disabled in PY32 v1
> peripheral). This sketch uses per-device sequential `latchPeriod()`
> with a single shared 50 ms settle. Skew at 100 kHz: ~810 µs total
> across 3 modules, < 0.0015 % of a 60 s period.
...
The library enforces the SPEC §10 two-step protocol — `prepareAddressChange()`
arms, `commitAddressChange()` writes within a 5-second window.
