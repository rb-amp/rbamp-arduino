...
Библиотека интегрирует по **часам ведущего**, а не по диагностическому
таймеру периода чипа (тот недосчитывает ~25-30 % под нагрузкой); при
пропущенном/устаревшем периоде якорь интегрирования удерживается, чтобы
следующее валидное окно не было недосчитано.
...
- **Конфигурация датчика тока** — `setSensorClass(class)` выбирает
  семейство (SCT-013 / встроенный CT / проводной CT), затем
  `setCTModel(code)` — или `configureChannels(class, models[], n)` для
  многоканального модуля одним вызовом. Принятый набор кодов SCT-013 —
  `{1=005A, 2=010A, 3=030A, 4=050A, 6=020A}`; `5=100A` и `7=060A`
  распознаются, но не охарактеризованы и отклоняются. Калибровочные
  коэффициенты загружаются автоматически из заводской таблицы.
- **Менеджер `RbAmpFleet`** — для нескольких модулей на одной шине:
  `scan()` находит и подключает их (с детекцией коллизий адресов),
  `pollAll()` читает за один проход, `totalPower()` / `totalEnergyWh()`
  агрегируют, а путь General-Call (`enableGcAll()` → `gcLatch(tick)` →
  `checkSync()`) защёлкивает период всех модулей одновременно с
  детекцией пропущенных кадров по каждому. `provision()` /
  `assignAddress()` вводят заводски-чистые модули на разные адреса.
- **Идентификация и здоровье** — `readVariant()`, `readCapability()`,
  `readUid()`, `readProductId()` и устойчивый канал событий/ошибок
  (`hasError()`, `clearError()`).
...
- **POD-структуры** `RbAmpSnapshot` / `RbAmpPeriodSnapshot` — все
  поля одного снимка в одной структуре. Поле, не прошедшее физический
  sanity-фильтр, ставится в `NaN` и отмечается в
  `RbAmpSnapshot::implausible` — остальной снимок остаётся пригодным.
- **Сокрытие протокольных деталей** — порядок байтов, времена
  выдержки после команд, NACK-retry (чтение и запись), защита от
  рваного чтения — всё внутри. Пользовательский код вызывает методы,
  а не пишет в регистры.

> **Надёжность на много-модульной / маргинальной шине (ESP32).** `Wire`
> на Arduino-ESP32 оборачивает драйвер, который может заблокироваться на
> удерживаемой шине *ниже* библиотеки — программный таймаут это не
> прервёт. Используйте внешние подтяжки **~4.7 кОм**, не держите
> отладчик/NRST на шине в продакшене и вооружите **прикладной task-WDT**
> на задаче опроса как путь восстановления. Эта трёхслойная дисциплина
> проверена расширенным soak-прогоном на стенде.
...
5. [`05_AddressChange`](examples/05_AddressChange/) — смена I2C-адреса модуля (two-phase commit)
6. [`06_BidirectionalEnergy`](examples/06_BidirectionalEnergy/) — раздельный учёт потребления и отдачи (мастер-сторонний)
7. [`07_DeepSleepLogger`](examples/07_DeepSleepLogger/) — логгер на батарейке с deep-sleep
8. [`08_FleetSync`](examples/08_FleetSync/) — скан флота, синхронизация периодов через General-Call, агрегация по флоту

Подробный разбор каждого скетча — в [docs/06_examples.md](docs/06_examples.md). Стендовый
валидационный харнесс лежит в [`extras/`](extras/) (это не Library-Manager-пример).

## Совместимость

Библиотека работает с прошивкой модуля **v1.0..v1.3**:
...
| **v1.3** | **`0x04`** | `REG_HW_VARIANT` / `REG_CAPABILITY` / UID, по-канальный CT, синхронизация флота через General-Call, two-phase смена адреса, канал событий |

Все версии библиотеки работают со всеми версиями прошивки.
Несуществующие на старой прошивке регистры возвращают `0x00`
(для байтовых) или `0.0f` (для float) — библиотека уходит в
соответствующий fallback (например, на прошивке до v1.3
`REG_HW_VARIANT` читается как `0x00`, и определение варианта идёт по
topology-подсказке из конструктора). Синхронизация флота через
General-Call требует прошивки v1.3; на более старой — fallback на
последовательный `latchPeriod()` по устройствам (см.
`03_MultiModuleBroadcast`).
